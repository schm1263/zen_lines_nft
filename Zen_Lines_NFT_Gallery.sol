...
contract ZenLinesRegistery is ERC721Full {
    ...
    function artworkRegistration(address owner, string memory name, string memory artist, uint initialValue, uint tokenURI) public returns(uint){
        uint tokenId = totalSupply();

        _mint(owner, tokenId);
        _setTokenURI(tokenId, tokenURI);

        gallery[tokenId] = ZenLines(name, artist, tokenURI);
